...
fn precedes_usr_bin(dir: &Path) -> bool {
    ...
    for (i, entry) in path.split(':').enumerate() {
        if Path::new(entry) == dir && found_dir.is_none() {
            found_dir = Some(i);
        }
        if entry == "/usr/bin" && found_usr.is_none() {
            found_usr = Some(i);
        }
    }
...
pub fn install(force_dir: Option<String>, force: bool) {
    let Some(self_p) = self_path() else {
        eprintln!(
            "❌ Could not locate the agent-git binary on PATH. Did you `cargo install agent-git`?"
        );
        return;
    };
    ...
    let Some(dir) = pick_dir(force_dir.as_deref()) else {
        ...
        for d in candidate_dirs() {
            eprintln!(
                "   - {} ({})",
                d.display(),
                if d.is_dir() { "exists" } else { "missing" }
            );
        }
        eprintln!();
        eprintln!("Fixes:");
        ...
        if !force {
            ...
            eprintln!(
                ...
                existing
                    ...
                    .unwrap_or_else(|| "(real file, not a symlink)".to_string())
            );
            eprintln!();
            eprintln!(
                "⚠️  WARNING: replacing /git interception is a SIGNIFICANT operation. It changes"
            );
            eprintln!(
                "   what every script that calls `git` sees. Re-run with --force if intentional."
            );
            return;
        }
...
pub fn uninstall(force_dir: Option<String>) {
    ...
    for dir in dirs {
        ...
        if target.symlink_metadata().is_ok() {
            ...
            let is_ours = link
                .as_deref()
                .map(|p| p.file_name().and_then(|n| n.to_str()) == Some(SELF_BIN))
                .unwrap_or(false);
            if is_ours {
                ...
                println!(
                    "⚪ {} exists but is not an agent-git symlink — left alone",
                    target.display()
                );
            }
        }
    ...
    if let Ok(out) = Command::new("bash").args(["-c", "command -v git"]).output() {
        ...
        println!(
            ...
            if ok {
                ...
                "❌ NOT ACTIVE — run `agent-git install`"
            }
        );
    }
}
...
fn print_doctor(dir: &Path, target: &Path) {
    ...
    println!();
    println!(
        "  {} on PATH before /usr/bin? {}",
        dir.display(),
        if precedes {
            "yes ✅"
        } else {
            "no ❌ — interception will NOT work"
        }
    );
    if !precedes {
        println!();
        println!(
            "  Fix your shell rc to put {} earlier in PATH, e.g.:",
            dir.display()
        );
